...
do, language, shift, message = ceasar.ask_input_question()

while True:
    if do:
        print(ceasar.code(language, shift, message))
    else:
        print(ceasar.decode(language, shift, message))
    ans = input('Нужно ли еще шифровать/дешифровать')
    if ans in ['нет', 'н', 'ytn', 'no', 'n', 'тщ', 'т']:
        break
    else:
        do, language, shift, message = ceasar.ask_input_question()
...
for i in range(len(text_list)):
    el_without_punct = text_list[i]
    for char in text_list[i]:
        if char in ['"', ',', '!', '?', ':', '.']:
            el_without_punct = text_list[i].replace(char, '')
    text_list[i] = code(False, len(el_without_punct), text_list[i])
